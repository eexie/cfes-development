...
/**
 * @copyright 2010-2013  Yann Dubois  ( email : yann _at_ abc.fr )
 *
 *  Original development of this plugin was kindly funded by http://www.yann.com
 *
...
if ( !class_exists( 'YD_Plugin' ) ) {
	...
	if( version_compare( PHP_VERSION, REQUIRED_PHP_VER ) >= 0 ) {
	    class YD_Plugin {
		    ...
		    public function __construct( $p_arr ) {
		    	// php5 constructor
				$this->YD_Plugin( $p_arr );
			}
			
			public function YD_Plugin( $p_arr ) {
				// Constructor
			...
			public function yd_add_menu_page() {
				add_menu_page( 
			...
			public function plugin_options() {
				if ( !current_user_can( 'manage_options' ) )  {
		   			wp_die( __('You do not have sufficient permissions to access this page.', 'wp-latest-posts') );
		  		}
			...
			private function update_options( &$op, $params ) {
		...
		if( !function_exists( 'yd_version_warning' ) ) {
			...
			function yd_version_warning() {
				...
				printf( __( '<strong>Error:</strong> The %s plugin', 'wp-latest-posts' ),  preg_replace( '/-/', ' ', $this_plugin ) );
				echo ' ';
				printf( __( 'Needs <strong>PHP %s</strong> or better.', 'wp-latest-posts' ), REQUIRED_PHP_VER );
				echo '</p></div>';
			}
		}
...
if ( !class_exists( 'YD_Widget' ) ) {
	class YD_Widget extends WP_Widget {
	    ...
	    private function display( $args, $instance ) {
	    }
...
if ( !class_exists( 'YD_OptionPage' ) ) {
	class YD_OptionPage {
		...
		private function logo_block() {
			echo '<div class="postbox">';
			echo '<h3 class="hndle">' . __( 'Considered donating?', 'wp-latest-posts' ) . '</h3>';
			...
			. '<br/><small>' . __( 'Enjoy this plugin?', 'wp-latest-posts' ) . '<br/>' . __( 'Help me improve it!', 'wp-latest-posts' ) . '</small><br/>'
			...
			. '" />'
			. '<input type="image" src="https://www.paypal.com/' . __( 'en_US', 'wp-latest-posts' ) . '/i/btn/btn_donateCC_LG.gif" name="submit" alt="PayPal - The safer, easier way to pay online!" />'
			. '<img alt="" border="0" src="https://www.paypal.com/' . __( 'en_US', 'wp-latest-posts' ) . '/i/scr/pixel.gif" width="1" height="1" />'
			. '</form>'
			. '<small><strong>' . __( 'Thanks', 'wp-latest-posts' ) . ' - Yann.</strong></small><br/><br/>';
			
			echo '</div>'; // / inside
			echo '</div>'; // / postbox
		}
		
		private function credits_block() {
			echo '<div class="postbox">';
			echo '<h3 class="hndle">' . __( 'Credits', 'wp-latest-posts' ) . '</h3>';
			...
			if( $this->additional_funding ) {
				foreach( $this->additional_funding as $funding ) {
					echo '<li>' . __( 'Additional:', 'wp-latest-posts' ) . 
			...
			echo __( 'If you want to contribute to a translation of this plugin, please drop me a line by ', 'wp-latest-posts' );
			echo '<a href="mailto:yann@abc.fr">' . __('e-mail', 'wp-latest-posts' ) . '</a> ';
			echo __( 'or leave a comment on the ', 'wp-latest-posts' );
			echo '<a href="' . $this->support_url . '">' . __( 'plugin\'s page', 'wp-latest-posts' ) . '</a>. ';
			echo __( 'You will get credit for your translation in the plugin file and the documentation page, ', 'wp-latest-posts' );
			echo __( 'as well as a link on this page and on my developers\' blog.', 'wp-latest-posts' );
				
			echo '</div>'; // / inside
			echo '</div>'; // / postbox
		}
		
		private function support_block() {
			echo '<div class="postbox">';
			echo '<h3 class="hndle">' . __( 'Support', 'wp-latest-posts') . '</h3>';
			echo '<div class="inside" style="padding:10px;">';
			echo '<b>' . __( 'Free support', 'wp-latest-posts' ) . '</b>';
			echo '<ul>';
			echo '<li>' . __( 'Support page:', 'wp-latest-posts' );
			echo ' <a href="' . $this->support_url . '">' . __( 'here.', 'wp-latest-posts' ) . '</a>';
			echo ' ' . __( '(use comments!)', 'wp-latest-posts' ) . '</li>';
			echo '</ul>';
			echo '<p><b>' . __( 'Professional consulting', 'wp-latest-posts' ) . '</b><br/>';
			...
			echo '</a>';
			echo __( 'I am available as an experienced free-lance Wordpress plugin developer and web consultant. ', 'wp-latest-posts' );
			echo __( 'Please feel free to <a href="mailto:yann@abc.fr">check with me</a> for any adaptation or specific implementation of this plugin. ', 'wp-latest-posts' );
			echo '<a href="http://www.yann.com/en/custom-developments">';
			echo __( 'Or for any WP-related custom development or consulting work. Hourly rates available.', 'wp-latest-posts' ) . '</a></p>';
			echo '</div>'; // / inside
			echo '</div>'; // / postbox
		}
		...
		public function form_header() {
			...
			echo '<form method="' . $fm . '" style="display:inline;" action="options.php">';
			settings_fields( $this->sanitized_name );
			/* deprecated with WP 2.7 Settings API
			 * @see http://codex.wordpress.org/Creating_Options_Pages
			 * 
			...
			*/
			do_settings_fields( $this->sanitized_name );
		}
		
		public function form_block( $name, $fields ) {
			echo '<div class="postbox">';
			echo '<h3 class="hndle">' . __( $name, $this->plg_tdomain ) . '</h3>';
			echo '<div class="inside">';
			echo '<table style="margin:10px;width:95%">';
			echo '<tr><th valign="top" align="left" class="settings">' . __('Setting:', 'wp-latest-posts') .
					'</th><th align="left" class="values">' . __('Value:', 'wp-latest-posts') . '</th></tr>';
			$submits = array();
			foreach( $fields as $field => $type ) {
		...
		public function form_action_button( $name, $action ) {
			echo '<p class="submit">';
		...
		public function form_footer() {
			...
			//echo '<input type="submit" class="yd_submit default" name="do" value="' . __('Update plugin settings', 'wp-latest-posts') . '" />';
			...
			echo '<input type="submit" class="yd_reset" name="do" value="' . __('Reset plugin settings', 'wp-latest-posts') . '" />';
			if( $this->has_cache ) {
				echo '<input type="submit" name="do" value="' . __('Clear cache', 'wp-latest-posts') . '" /><br/>';
			}			
			echo '</span>'; // / submit
		...
		public function cron_status( $crontab ) {
		...
		private function do_action( &$yd_plugin ) {
			...
			if( !$done ) {
				if(			$p["do"] == __('Clear cache', 'wp-latest-posts') ) {
					$yd_plugin->clear_cache();
					$this->update_msg = '<p>' . __('Caches are cleared', 'wp-latest-posts') . '</p>';
				} elseif(	$p["do"] == __('Reset plugin settings', 'wp-latest-posts') ) {
					$yd_plugin->reset_options();
					$this->update_msg = '<p>' . __('Plugin settings are reset', 'wp-latest-posts') . '</p>';
				} elseif(	$p["do"] == __('Update plugin settings', 'wp-latest-posts') ) {
					$yd_plugin->update_options( $this, $p );
					$this->update_msg = '<p>' . __('Plugin settings are updated', 'wp-latest-posts') . '</p>';
				} else {
