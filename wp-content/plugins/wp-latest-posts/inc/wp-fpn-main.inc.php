...
class wpcuWPFnPlugin extends YD_Plugin {
            ...
            if (preg_match('/^wpcufpn_/', $field_name)) {
                ...
                if (is_uploaded_file($_FILES[$field_name]['tmp_name'])) {
                    ...
                    if (rename($_FILES[$field_name]['tmp_name'], $upload_dir . $upload_file)
                        ...
                        // $this->update_msg .= __( 'Temporary file ', 'wp-latest-posts') . $_FILES["game_image"]["tmp_name"] .
    ...
    public function setupCustomMenu() {
        add_submenu_page(
                'edit.php?post_type=wpcuwpfp-news-widget', __('Configuration', 'wp-latest-posts'), __('Configuration', 'wp-latest-posts'), 'activate_plugins', 'translation-wpfpn', array($this, 'displayConfigTab')
        );
    }
        ...
        <?php endforeach; ?>
			...
			<a id="wpcufpn_dateFormat" target="_blank" href="http://php.net/manual/en/function.date.php"> ' . __('Date format', 'wp-latest-posts') . ' </a>
                    ...
                    if (isset($post->ID) && isset($post->post_title) && (!empty($post->post_title))) {
                        ...
                        '<br><textarea readonly id="phpCodeInsert" cols="100" rows="2" name="wpcufpn_phpCodeInsert">' . 'echo do_shortcode(\'[frontpage_news widget="' . $post->ID . '" name="' . $post->post_title . '"]\');' . '</textarea></div>';
                    }
                }
				...
				<td>
                    ...
                    echo '<p>' . __('Initially released in october 2013 by <a href="http://www.joomunited.com/">JoomUnited</a>', 'wp-latest-posts') . '</p>';
                    ...
                    echo '<p>' . __('Your hosting provider\'s web server currently runs: ', 'wp-latest-posts') . $_SERVER['SERVER_SOFTWARE'] . '</p>';
                    echo '<p><em>' . __('Please specify all of the above information when contacting us for support.', 'wp-latest-posts') . '</em></p>';
                ...
                public function displayConfigTab(){
                    ...
                    wp_register_style('wpcufpnAbout', plugins_url('css/wpcufpn_about.css', dirname(__FILE__)));
                    wp_enqueue_style('wpcufpnAbout');
                    ?>
                ...
                private function displayContentSourceCategoryTab() {
                    ...
                    if(is_multisite()){
			            ...
			            <label for="mutilsite_select_post" class="post_cb">'.__( 'Multisite selection', 'wp-latest-posts' ).' : </label>		
                ...
                private function displayContentSourcePageTab() {
                    ...
                    if(is_multisite()){
                        ...
                        echo '<li class="field "> 
                        <div class="width33 input-field input-select">
			            <label for="mutilsite_select_page" class="page_cb">'.__( 'Multisite selection', 'wp-latest-posts' ).' : </label>		
